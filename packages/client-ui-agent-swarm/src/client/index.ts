...
import { SwarmTrajectoryPanel, type AgentSwarmInjected } from './SwarmTrajectoryPanel.js'
import type { SwarmAttemptData } from './trajectory-definition.js'
import { swarmTrajectoryDefinition } from './trajectory-definition.js'
import { en, NS, type AgentSwarmLocaleKey, zh } from './locales.js'
import { installStyles } from './styles.js'

declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface LocaleNamespaceMap {
    agentSwarm: AgentSwarmLocaleKey
  }
}

export type { AgentSwarmInjected, SwarmTrajectoryPanelProps } from './SwarmTrajectoryPanel.js'
export type {
  SwarmAttemptData,
  SwarmTaskNodeData,
...
/** Cordis services required by the Agent Swarm browser companion. */
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'ui-agent-swarm: dictionaries')
  ctx.effect(installStyles, 'ui-agent-swarm: styles')
  ctx.slots.inject('conversation.chat.node', () => ctx.slots.register({
    name: 'conversation.chat.node',
    key: 'agent-swarm-trajectory',
    locale: NS,
    inject: (): AgentSwarmInjected => ({
