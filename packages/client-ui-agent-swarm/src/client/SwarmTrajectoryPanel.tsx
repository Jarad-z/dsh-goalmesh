...
import type { AgentSwarmLocaleKey } from './locales.js'
import type {
  SwarmAttemptData,
...
export interface AgentSwarmInjected {
  readonly openAgent: (attempt: SwarmAttemptData) => Promise<boolean>
}

export type SwarmTrajectoryPanelProps =
  PropsRuntime<'conversation.chat.node', 'agent-swarm-trajectory'>
  & PropsLocale<'agentSwarm'>
  & AgentSwarmInjected
...
} as const satisfies Record<DisplayStatus, AgentSwarmLocaleKey>
...
export function SwarmTrajectoryPanel({ node, openAgent, t, useSessions }: SwarmTrajectoryPanelProps) {
  ...
  return (
    <section className="dsh-swarm" data-agent-swarm-trajectory data-run-status={node.data.status}>
      <button
        type="button"
