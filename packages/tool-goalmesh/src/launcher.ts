...
import { installChildGoalMeshTool } from './nested-tool.js'
...
export const ATTEMPT_TIMEOUT_CODE = 'GOAL_MESH_ATTEMPT_TIMEOUT'
...
export class SubagentLauncher implements Launcher {
  ...
  async start(task: MaterializedTask): Promise<LaunchedTask> {
    ...
    const run: SubagentRun = await this.ctx.subagents.start(this.config.provider, {
      ...
      ...lease === undefined ? {} : {
        scopedSetup: (childCtx: Context) => { installChildGoalMeshTool(childCtx, lease, this.config) },
      },
    })
