...
const PACKAGE_NAME = 'dsh-tool-goalmesh'
...
export const name = 'goalmesh-invariant'
...
function isSwarmEvent(event: SessionEvent): boolean {
  return event.type.startsWith('tool-goalmesh/')
}
...
function cloneTrace(source: TrajectoryTrace, event: SessionEvent, fail: InvariantFailure): TrajectoryTrace {
  ...
  if (event.type === 'tool-goalmesh/run-start') return trace
...
export function applyTrajectoryEvent(
  ...
  if (!EVENT_TYPES.has(event.type)) fail(`unknown GoalMesh trajectory event type ${event.type}`)
  if (event.ignorable !== true) fail(`${event.type} must carry ignorable: true`)
  const data = recordOf(event, fail)
  const swarmId = stringId(data.swarmId, `${event.type} swarmId`, fail)

  switch (event.type) {
    case 'tool-goalmesh/run-start': {
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (candidate === undefined || candidate.session !== session) {
      return fail('session/event reached publication without matching GoalMesh trajectory validation')
    }
    staged.delete(event)
