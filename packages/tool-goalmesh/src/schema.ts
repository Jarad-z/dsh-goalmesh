...
export const ConfigSchema = z.object({
  ...
  toolName: z.string().default('goal_mesh'),
  ...
  swarmTimeoutMs: z.natural().min(0).max(MAX_TIMER_DELAY_MS).default(0),
  attemptTimeoutMs: z.natural().min(0).max(MAX_TIMER_DELAY_MS).default(0),
  maxTaskReportChars: z.natural().min(0).max(Number.MAX_SAFE_INTEGER).default(0),
  maxRenderedResultChars: z.natural().min(0).max(Number.MAX_SAFE_INTEGER).default(0),
...
export const ROOT_GOAL_MESH_PARAMETERS = {
  goal: {
...
export const NESTED_GOAL_MESH_PARAMETERS = {
  tasks: ROOT_GOAL_MESH_PARAMETERS.tasks,
  failure_mode: ROOT_GOAL_MESH_PARAMETERS.failure_mode,
  quorum: ROOT_GOAL_MESH_PARAMETERS.quorum,
} as const
...
export const GOAL_MESH_OUTPUT_SCHEMA = {
  type: 'object' as const,
  additionalProperties: false,
  properties: {
