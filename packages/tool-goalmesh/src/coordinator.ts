...
export const SWARM_TIMEOUT_CODE = 'GOAL_MESH_TIMEOUT'
...
interface InvocationState {
  ...
  readonly completion: Deferred<GoalMeshToolValue>
  ...
  terminalReason: GoalMeshToolValue['terminalReason']
...
export class SwarmCoordinator {
  ...
  invokeRoot(input: RootInvocationInput): InvocationHandle {
    if (!this.accepting) throw new Error('goal_mesh is shutting down and no longer accepts invocations')
    ...
    if (existingRef !== undefined) {
      ...
      if (existingRun === undefined || existingInvocation === undefined) {
        throw new Error('goal_mesh command index references a released invocation')
      }
    ...
    if (!provider.capabilities.outputSchema || !provider.capabilities.depthLimit) {
      throw new Error(
        `subagent provider "${provider.name}" must support outputSchema and depthLimit for goal_mesh v0.3`,
      )
    }
    ...
    if (rootDepth !== 0) {
      throw new Error('the global goal_mesh Tool accepts only top-level callers; nested callers require a scoped lease')
    }
    const absoluteMaxDepth = rootDepth + this.config.maxDepth
    if (!Number.isSafeInteger(absoluteMaxDepth)) throw new Error('goal_mesh absolute maxDepth exceeds safe integer range')
    ...
    const onAbort = (): void => {
      const cancellation: RunCancellation = timeoutOf(swarmDeadline.signal, SWARM_TIMEOUT_CODE) !== undefined
        ? { kind: 'deadline_exceeded', message: 'goal_mesh exceeded its swarm deadline' }
        : controller.signal.aborted && controller.signal.reason instanceof SwarmRunError
          ...
          : { kind: 'cancelled', message: 'goal_mesh was cancelled by its caller' }
    ...
    recorder.append('tool-goalmesh/run-start', {
      swarmId,
      rootSessionId: input.rootAgent.session.id,
      goalSummary: boundedText(goal.statement, 500),
    })
    recorder.append('tool-goalmesh/invocation-start', {
      swarmId,
      invocationId,
  ...
  dispose(): Promise<void> {
    ...
    this.disposal ??= (async () => {
      ...
      for (const run of this.runs.values()) {
        ...
        this.enqueue(run, {
          kind: 'cancel-run',
          cancellation: { kind: 'plugin_disposed', message: 'goal_mesh plugin was disposed' },
        })
  ...
  private createInvocation(input: {
    ...
    const completion = deferred<GoalMeshToolValue>()
  ...
  private registerTasks(
    run: RunState,
    invocation: InvocationState,
    sources: readonly GoalMeshTaskV02[],
    ...
    sources.forEach((source, index): void => {
      ...
      if (taskId === undefined) throw new Error('goal_mesh failed to allocate an admitted task')
    ...
    for (const taskId of taskIds) {
      ...
      if (task === undefined) throw new Error('goal_mesh failed to register an admitted task')
      for (const dependencyId of task.spec.dependencies) run.dependents.get(dependencyId)?.push(taskId)
      run.recorder.append('tool-goalmesh/task-created', {
        swarmId: run.id,
        invocationId: invocation.id,
        taskId,
  ...
  private invokeNested(swarmId: SwarmId, token: LeaseToken, input: NestedInvocationInput): InvocationHandle {
    if (!this.accepting) throw new Error('goal_mesh is shutting down and no longer accepts nested invocations')
    const run = this.runs.get(swarmId)
    if (run === undefined || run.phase === 'finished') throw new Error('nested goal_mesh lease references an inactive Swarm')
    ...
    if (lease === undefined) throw new Error('nested goal_mesh lease is revoked or unknown')
    if (lease.callerAgent !== input.callerAgent) throw new Error('nested goal_mesh caller does not match the leased child')
    ...
    if (attempt === undefined || parentTask === undefined
      ...
      throw new Error('nested goal_mesh lease failed attempt fencing')
    }
    if (parentTask.phase !== 'running' || !attempt.holdsPermit) {
      throw new Error(`nested goal_mesh parent task is ${parentTask.phase}, expected running with a permit`)
    }
    ...
    if (input.signal.aborted) throw new Error('nested goal_mesh was cancelled before admission')
      ...
      const cancellation: RunCancellation = parentTimedOut || swarmTimedOut
        ? {
            ...
            message: parentTimedOut
              ...
              : 'goal_mesh exceeded its swarm deadline while nested work was active',
          }
        : { kind: 'cancelled', message: 'nested goal_mesh invocation was cancelled' }
    ...
    run.recorder.append('tool-goalmesh/invocation-start', {
      swarmId: run.id,
      invocationId,
      parentTaskId: parentTask.spec.id,
  ...
  private issueLease(run: RunState, task: TaskState, attemptId: AttemptId): ScopedSwarmLease {
    const token = Symbol('goalmesh-lease') as LeaseToken
  ...
  private bindLease(swarmId: SwarmId, token: LeaseToken, agent: Agent): void {
    ...
    if (run === undefined || record === undefined || run.phase === 'finished') {
      throw new Error('cannot bind a revoked GoalMesh lease')
    }
    if (record.callerAgent !== undefined && record.callerAgent !== agent) {
      throw new Error('GoalMesh lease is already bound to a different child')
    }
    ...
    return {
      ...
      dispose: async (): Promise<void> => {
        ...
        if (run.handleCount < 0) {
          run.handleCount = 0
          throw new Error('goal_mesh invocation handle released more than once')
        }
  ...
  private drain(run: RunState): void {
    ...
    try {
      ...
      this.ctx.logger.error(`goal_mesh state corruption in ${run.id}: ${sanitizeDiagnostic(error)}`)
      this.cancelRunCommitted(run, {
        kind: 'state_corrupted',
        message: `goal_mesh state corrupted: ${sanitizeDiagnostic(error)}`,
      }, effects)
      this.finishIfComplete(run)
    } finally {
      run.draining = false
  ...
  private onPublished(run: RunState, attemptId: AttemptId, launched: LaunchedTask, effects: (() => void)[]): void {
    ...
    run.recorder.append('tool-goalmesh/attempt-start', {
      swarmId: run.id,
      taskId: task.spec.id,
      attemptId,
  ...
  private settleAttempt(
    ...
    run.recorder.append('tool-goalmesh/attempt-end', {
      swarmId: run.id,
      taskId: task.spec.id,
  ...
  private revokeLeaseCommitted(
    ...
    for (const invocationId of task.openChildInvocationIds) {
      this.cancelInvocationCommitted(run, invocationId, {
        kind: 'cancelled',
        message: `nested GoalMesh lease was revoked: ${boundedText(reason, 300)}`,
      }, effects)
    }
  }
  ...
  private transition(
    ...
    task.phase = to
    task.viewStatus = to
    run.recorder.append('tool-goalmesh/task-transition', {
      swarmId: run.id,
      taskId: task.spec.id,
      from,
      to,
  ...
  private maybeStopByPolicy(
    ...
    if (invocation.failureMode === 'fail_fast' && terminal.kind === 'failed') {
      this.stopByPolicy(run, invocation, {
        reason: 'failed_fast',
        message: 'goal_mesh stopped this invocation after the first task failure',
      }, effects)
      return true
    }
    if (invocation.failureMode === 'quorum' && terminal.kind === 'completed') {
      ...
      if (completed >= (invocation.quorum ?? Number.MAX_SAFE_INTEGER)) {
        this.stopByPolicy(run, invocation, {
          ...
          message: `goal_mesh invocation reached quorum (${completed}/${invocation.quorum})`,
        }, effects)
        return true
      }
    }
  ...
  private settleInvocation(run: RunState, invocation: InvocationState): void {
    ...
    run.recorder.append('tool-goalmesh/invocation-end', {
      swarmId: run.id,
      invocationId: invocation.id,
      status,
    })
  ...
  private auditIfStalled(run: RunState): void {
    ...
    this.ctx.logger.warn(`goal_mesh dependency deadlock audit settled ${stalledIds.length} tasks in ${run.id}`)
    const effects: (() => void)[] = []
  ...
  private finishIfComplete(run: RunState): void {
    ...
    run.recorder.append('tool-goalmesh/run-end', {
      swarmId: run.id,
      status,
      ...eventSummary,
    })
  ...
  private toolValue(run: RunState, invocation: InvocationState): GoalMeshToolValue {
...
export async function settleInvocationHandle(handle: InvocationHandle): Promise<GoalMeshToolValue> {
  ...
  if (execution.status === 'rejected') {
    if (cleanup.status === 'rejected') {
      throw new AggregateError([execution.reason, cleanup.reason], 'goal_mesh invocation and handle cleanup failed')
    }
