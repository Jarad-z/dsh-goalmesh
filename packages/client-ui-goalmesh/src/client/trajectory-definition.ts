...
export interface SwarmTimelineEntry {
  ...
  readonly type: GoalMeshEventType
...
interface SwarmInvocationState {
  ...
  readonly status?: GoalMeshEventMap['tool-goalmesh/invocation-end']['status']
}
...
export interface SwarmTrajectoryState {
  ...
  readonly runEnd?: GoalMeshEventMap['tool-goalmesh/run-end']
...
declare module '@deepseek-ai/dsh-client-ui-conversation/client' {
  interface ChatNodeDataMap {
    'goalmesh-trajectory': SwarmTrajectoryChatData
  }
}

type SwarmUpdateEventType = Exclude<GoalMeshEventType, 'tool-goalmesh/run-start'>
...
export function isSwarmUpdateEvent(event: SessionEvent): event is SessionEvent<SwarmUpdateEventType> {
  switch (event.type) {
    case 'tool-goalmesh/invocation-start':
    case 'tool-goalmesh/task-created':
    case 'tool-goalmesh/attempt-start':
    case 'tool-goalmesh/task-transition':
    case 'tool-goalmesh/attempt-end':
    case 'tool-goalmesh/invocation-end':
    case 'tool-goalmesh/run-end':
      return true
    default:
      return false
  }
...
/** Durable GoalMesh event family folded into one keyed Chat node. */
export const swarmTrajectoryDefinition: ConversationNodeDefinition<SwarmTrajectoryState> = {
  ...
  start(_context, match) {
    if (match.event.type !== 'tool-goalmesh/run-start') {
      throw new Error('goalmesh-trajectory start requires tool-goalmesh/run-start')
    }
    return {
